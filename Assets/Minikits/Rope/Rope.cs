...
namespace RopeMinikit
{
    public class Rope : MonoBehaviour
    {
        ...
        [Tooltip("The radius of the rope. This value is used to build the visual mesh and handle collisions.")]
        [Range(0.001f, 1.0f)]
        public float radius = 0.05f;

        [Tooltip(
            "The number of vertices used for each segment of the rope visible mesh. More vertices will cause the rope to appear rounder, but will increase the overall vertex and triangle count of the visual mesh. This value does not affect the simulation of the rope at all.")]
        [DisableInPlayMode]
        [Range(3, 32)]
        public int radialVertices = 6;

        [Tooltip(
            "Whether the rope is a circular loop. If enabled, the rope's last spawn point will be connected to the first spawn point.")]
        [DisableInPlayMode]
        public bool isLoop = false;

        [Tooltip(
            "The material used to render the rope. This can be any material that uses vertex positions and optionally normals.")]
        ...
        [Tooltip(
            "When using a custom mesh, this property specifies how much to rotate the mesh around the z-axis for every link in the chain of simulation particles.")]
        [Range(0.0f, 360.0f)]
        public float customMeshRotation = 90.0f;

        [Tooltip(
            "Used to initially place the rope at the spawn point in the world. Currently, pairs of consecutive generated points are considered linear segments.")]
        [DisableInPlayMode]
        ...
        {
            [Tooltip("Turns the simulation on or off independently of the rendering of the rope.\n " +
                     "A use case might be to programmatically disable ropes that are too far from the camera or ropes that are not visible.")]
            public bool enabled;

            [Tooltip(
                "The number of simulated particles per meter. Higher resolutions will make the ropes look smoother, but require more computation.")]
            [DisableInPlayMode]
            public float resolution;

            [Tooltip(
                "The number of solver iterations to run for this rope. High-res ropes require more iterations to get stiff. More iterations require more computation.")]
            [Range(1, 32)]
            public int solverIterations;

            [Tooltip(
                "Quality of rope per meter. when passed RopeRigidbodyConnection this value is used when the component interacts with the rigid body.")]
            [Delayed]
            public float massPerMeter;

            [Tooltip(
                "A measure of rope stiffness.\n Note that the actual stiffness strongly depends on the number of solver iterations and the size of the physics time step used" +
                "If you change one value, you may need to readjust other values." +
                " This specific value does not affect performance.")]
            [Range(0.01f, 1.0f)]
            public float stiffness;

            [Tooltip(
                "Dynamically shorten or lengthen the value of the rope by a multiplicative factor. This can be used to create retractable grappling hooks, for example.")]
            [Range(0.0f, 2.0f)]
            public float lengthMultiplier;

            [Tooltip(
                "Percentage of energy removed from the simulation per fixed update. Used to simulate air resistance. Does not affect performance.")]
            [Range(0.0f, 1.0f)]
            public float energyLoss;

            [Tooltip(
                "The percentage of gravity applied to the rope. The low gravity multiplier might help straighten out the rope that would otherwise get sacked, but should be considered a \"hack\" because the rope behaves like it's in space.")]
            [Range(0.0f, 1.0f)]
            public float gravityMultiplier;

            [Tooltip("Gravity used for this special rope")]
        ...
        {
            [Tooltip(
                "Enables collision handling for the rope so that it reacts to colliders other than those connected via the RopePins or RopeRigidbodyConnection components. " +
                "Performance intensive on the main thread.")]
            public bool enabled;

            [Tooltip(
                "Check for and respond to collisions on every nth simulated particle\n " +
                "A value of 1 will make each simulated particle react to collisions\n " +
                "A value of 2 will make all other particles react to collisions, and so on\n " +
                "Because each particle performs a sphere overlap test, lower values are important for performance\n " +
                "When a rope is selected, colliding particles are shown by yellow spheres.")]
            [Range(1, 20)]
            public int stride;

            [Tooltip(
                "The coefficient of kinetic friction of the rope. For example, if the rope is dragged along the ground, it is used to slow down the rope.")]
            [Range(0.0f, 20.0f)]
            public float friction;

            [Tooltip(
                "An extra distance (added on top of the rope radius) that prevents small radius ropes from easily passing through geometry")]
            [Range(0.0f, 1.0f)]
            ...
            for (int i = 0; i < activeCollisionPlanes.Length; i++)
            {
                ...
                var hitCount =
                    Physics.OverlapSphereNonAlloc(pos, extendedRadius, collisionQueryBuffer,
                        layerMask); // use a slightly larger sphere to catch more collisions
                for (int j = 0; j < hitCount && planeCount < MaxCollisionPlanesPerParticle; j++)
                {
                    var collider = collisionQueryBuffer[j];
                    var meshCollider = collider as MeshCollider;

                    if ((meshCollider != null && meshCollider.convex) || collider is BoxCollider ||
        ...
        protected void ApplyRigidbodyConnectionFeedback()
        {
            ...
            for (int i = 0; i < iterationCount; i++)
            {
                ...
                if (c.rigidbody)
                {
                    ...
                    {
                        var impulse = particleTargetFeedbacks[i] *
                                      (particleMass * massMultiplier * invDtAndSolverIterations);
        ...
        protected void ScheduleNextSimulationFrame()
        {
            ...
            var integrateParticles = new IntegrateParticlesJob()
            {
                ...
                externalAcceleration = simulation.gravity * simulation.gravityMultiplier,
        ...
        protected void SubmitToRenderer()
        {
            ...
            else
            {
                ...
                for (var i = 0;
                    i < positions.Length;
                    i++) // use the positions array here and hope that the compiler optimizes away bounds checking...
                {
                    ...
                    var rotation = Quaternion.LookRotation(tangent, bitangents[i]) *
